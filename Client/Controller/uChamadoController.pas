...
//  rel := TDMRelChamado.Create(nil);
  ...
  //    rel.CDSRelChamadoModelo5.Close;
      Negocio.RelatorioChamado(AModelo, AIdUsuario, oObjetoJSON, AOrdem, Integer(caChamado));
  //    rel.lblPeriodoModelo5.Caption := 'Período de ' + AFiltro.DataInicial + ' Até ' + AFiltro.DataFinal;
  //    rel.RelChamadoModelo5.Print;
      Grafico.DBChart1.Title.Text.Clear;
      Grafico.DBChart1.Title.Text.Add('Chamados - Estatística de Chamados e Horas por Período');
...
// serializa o objeto
  ...
  try
    try
      ...
      if AFiltro.TipoMovimento = 1 then
      begin
        rel.lblTituloRelatorio7.Caption := 'Chamados - Resumo de Ocorrências por Usuário Mês';
        iTipo := Integer(caChamado);
      end;

      if AFiltro.TipoMovimento = 2 then
      begin
        rel.lblTituloRelatorio7.Caption := 'Atividades - Resumo de Ocorrências por Usuário Mês';
...
//    raise Exception.Create('Informe o Usuário de Abertura!');
//
...
//    raise Exception.Create('Informe a Descrição na Abertura!');
...
begin
  ...
  if StrToIntDef(IdStatus, 0) = 0 then
    raise Exception.Create('Informe o Status Atendimento na Tabela de Parâmetros !');
