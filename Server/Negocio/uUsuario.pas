...
type
  ...
  public
    ...
    procedure ObterUsuarioDepartamento(AUsuarioDepartamento: TUsuarioDepartamentoViewModel; AIdUsuario: Integer);
...
procedure TUsuario.ObterUsuarioDepartamento(
  AUsuarioDepartamento: TUsuarioDepartamentoViewModel; AIdUsuario: Integer);
var
  lQry: TFDQuery;
begin
  lQry := TFDQuery.Create(nil);
  try
    lQry.Connection := dm.Conexao;
    lQry.SQL.Add('select');
    lQry.SQL.Add('	usu.Usu_Codigo,');
    lQry.SQL.Add('	Dep.Dep_ModeloImprOrcamento');
    lQry.SQL.Add('from Usuario usu');
    lQry.SQL.Add('	INNER JOIN Departamento Dep On usu.Usu_Departamento = Dep.Dep_Id');
    lQry.SQL.Add('where usu.Usu_Id = ' + IntToStr(AIdUsuario));
    lQry.Open();

    AUsuarioDepartamento.CodUsuario := lQry.FieldByName('Usu_Codigo').AsInteger;
    AUsuarioDepartamento.ModeloImpressaoOrcamento := lQry.FieldByName('Dep_ModeloImprOrcamento').AsInteger;
  finally
    FreeAndNil(lQry);
  end;
end;
