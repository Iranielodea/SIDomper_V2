...
function TServerModule2.OrcamentoNovo(AIdUsuario: Integer): TJSONValue;
var
  obj: TOrcamento;
  model: TOrcamentoViewModel;
  tipo: TEnumPrograma;

  Usuario: TUsuario;
  UsuarioDep: TUsuarioDepartamentoViewModel;

  observacaoModel: TObservacaoVO;
  observacao: TObservacao;

  tipoModel: TTipoVO;
  tipoRepositorio: TTipo;

  usuarioModel: TUsuarioVO;
begin
  Usuario := TUsuario.Create;
  UsuarioDep := TUsuarioDepartamentoViewModel.Create;

  observacaoModel := TObservacaoVO.Create;
  observacao := TObservacao.Create;

  obj := TOrcamento.Create;
  model := TOrcamentoViewModel.Create;

  tipoRepositorio := TTipo.Create;
  try
    model.Codigo := obj.CodigoAtual();
    Usuario.ObterUsuarioDepartamento(UsuarioDep, AIdUsuario);

    model.ModeloImpressao := UsuarioDep.ModeloImpressaoOrcamento;

    if model.ModeloImpressao = 1 then
      tipo := prOrcamento
    else
      tipo := prOrcamentoAgroD;

    // observacao padrao
    observacao.LocalizarPadrao(observacaoModel, Integer(tipo));
    model.ObservacaoPadrao := observacaoModel.Descricao;

    // email padrao
    observacao.LocalizarEmailPadrao(observacaoModel, Integer(tipo));
    model.TextoEmailPadrao := observacaoModel.ObsEmail;

    // tipo
    tipoModel := tipoRepositorio.RetornoUmRegistro(Integer(prOrcamento));
    if tipoModel.Id > 0 then
    begin
      model.IdTipo := tipoModel.Id;
      model.CodTipo := tipoModel.Codigo;
      model.NomeTipo := tipoModel.Nome;
    end;

    usuarioModel := Usuario.LocalizarId(AIdUsuario);
    model.IdUsuario := usuarioModel.Id;
    model.CodUsuario := usuarioModel.Codigo;
    model.NomeUsuario := usuarioModel.Nome;

    Result := TConverte.ObjectToJSON<TOrcamentoViewModel>(model);
  finally
    obj.DisposeOf;
    Usuario.DisposeOf;
    UsuarioDep.DisposeOf;
    observacaoModel.DisposeOf;
    observacao.DisposeOf;
    tipoModel.DisposeOf;
    tipoRepositorio.DisposeOf;
    usuarioModel.DisposeOf;
    model.DisposeOf;
  end;
end;
