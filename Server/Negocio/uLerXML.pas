...
//    procedure GravarEmails(Cliente: TClienteVO; listaEmail: TObjectList<TEmailVO>);
    ...
    procedure LocalizarClienteCNPJ_CPF(ACliente: TClienteVO; ADocumento: string); overload;

    function LocalizarClienteCNPJ_CPF(ADocumento: string; var ALatitude: string; var ALongitude: string): Integer; overload;

    function LocalizarClienteCodigo(Codigo: integer; var ALatitude: string; var ALongitude: string): Integer;
...
begin
  sLongitude := '';
  sLongitude := '';
//  Cliente.Id := LocalizarClienteCodigo(Cliente.Codigo, sLatitude, sLongitude);
  LocalizarClienteCNPJ_CPF(Cliente, Cliente.CNPJ);
  Cliente.Latitude := sLatitude;
  Cliente.Longitude := sLongitude;
  ...
  try
    if Cliente.Id = 0 then
      Cliente.Codigo := objCliente.ProximoCodigo();
...
//    iCont := Cliente.ClienteModulo.Count;
    ...
    if Cliente.ClienteModulo.Count > 0 then
        ExcluirModuloProduto(Cliente.Id);
    try
      ...
      begin
        ...
        begin
          if (Cliente.Id > 0) then
          begin
            Item.Modulo.Id := LocalizarModuloCodigo(Item.Modulo.Codigo);
            Item.Produto.Id := LocalizarProdutoCodigo(Item.Produto.Codigo);

            if (Item.Modulo.Id > 0) then
            begin
              Qry.Close;
              Qry.ParamByName('IdCliente').AsInteger := Cliente.Id;
              Qry.ParamByName('IdModulo').AsInteger := Item.Modulo.Id;

              if Item.Produto.Id > 0 then
                Qry.ParamByName('IdProduto').AsInteger := Item.Produto.Id
              else begin
...
function TLerXML.LocalizarClienteCNPJ_CPF(ADocumento: string; var ALatitude,
  ALongitude: string): Integer;
var
  Qry: TFDQuery;
  i: Integer;
  iCont: Integer;
  sDocumento: string;
begin
  sDocumento := SomenteNumeros(ADocumento);

  if sDocumento.Length = 11 then
    sDocumento := formatMaskText('999.999.999-99;0;_', sDocumento)
  else
    sDocumento := formatMaskText('99.999.999/9999-99;0;_', sDocumento);

  Qry := TFDQuery.Create(nil);
  try
    Qry.Connection := dm.Conexao;
    Qry.SQL.Text := 'SELECT Cli_Id, Cli_Latitude, Cli_Longitude FROM Cliente WHERE Cli_Dcto = ' + QuotedStr(sDocumento);
    Qry.Open();

    ALatitude := Qry.Fields[1].AsString;
    ALongitude := Qry.Fields[2].AsString;
...
procedure TLerXML.LocalizarClienteCNPJ_CPF(ACliente: TClienteVO; ADocumento: string);
var
  Qry: TFDQuery;
  i: Integer;
  iCont: Integer;
...
begin
  ...
  try
    Qry.Connection := dm.Conexao;
    Qry.SQL.Text := 'SELECT Cli_Id, Cli_Codigo, Cli_Latitude, Cli_Longitude FROM Cliente WHERE Cli_Dcto = ' + QuotedStr(sDocumento);
    Qry.Open();

    ACliente.Id := Qry.FieldByName('Cli_Id').AsInteger;
    ACliente.Codigo := Qry.FieldByName('Cli_Codigo').AsInteger;
    ACliente.Latitude := Qry.FieldByName('Cli_Latitude').AsString;
    ACliente.Longitude := Qry.FieldByName('Cli_Longitude').AsString;
  finally
    FreeAndNil(Qry);
  end;
end;

function TLerXML.LocalizarClienteCodigo(Codigo: integer; var ALatitude: string; var ALongitude: string): Integer;
var
  Qry: TFDQuery;
  i: Integer;
  iCont: Integer;
begin
  ...
  try
    Qry.Connection := dm.Conexao;
    Qry.SQL.Text := 'SELECT Cli_Id, Cli_Latitude, Cli_Longitude FROM Cliente WHERE Cli_Codigo = ' + Codigo.ToString;
    Qry.Open();
...
begin
  ...
  try
    ...
    if Cliente.Revenda = 0 then
      ListaErros.Add('Revenda.............: - Não Informado')
    else begin
    ...
    if Cliente.PendenciaFinanceira.Trim() = '' then
      ListaErros.Add('Pendência Financeira: - Não Informado');

    if Cliente.Situacao.Trim() = '' then
      ListaErros.Add('Situação............: - Não Informado');
