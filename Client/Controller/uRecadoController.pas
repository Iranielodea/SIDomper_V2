...
type
  ...
  public
    ...
    procedure EnviarListaEmail(ARecados: TObjectList<TRecadoVO>);
...
procedure TRecadoController.EnviarListaEmail(ARecados: TObjectList<TRecadoVO>);
var
  objContaEmail: TContaEmailController;
  Usuario: TUsuarioController;
  sEmail: string;
  ...
  item: TRecadoVO;
  oEmail: TEmailBase;
  listaEmail: TObjectList<TEmailBase>;
begin
  Usuario := TUsuarioController.Create;
  objContaEmail := TContaEmailController.Create;
  listaEmail := TObjectList<TEmailBase>.Create();
  try
    for item in ARecados do
    begin
      if (item.Id = 0) or (item.IdUsuarioLcto = 0) or (item.IdUsuarioDestino = 0) then
        continue;

      sAssunto := 'Domper Recado - ' + FormatFloat('000000', item.Id);

      Usuario.LocalizarId(item.IdUsuarioDestino);
      sEmail := Usuario.Model.CDSCadastroUsu_Email.AsString;

      if sEmail.Trim <> '' then
      begin
        oEmail := TEmail.Create;
        oEmail.IdUsuario := item.IdUsuarioLcto;
        oEmail.Destinatario := sEmail;
        oEmail.Assunto := sAssunto;
        oEmail.Texto := TextoEmail(item.Id);

        listaEmail.Add(oEmail);
      end;
    end;

    if listaEmail.Count > 0 then
      objContaEmail.EnviarEmails(listaEmail);
  finally
    FreeAndNil(objContaEmail);
    FreeAndNil(Usuario);
    listaEmail.DisposeOf;
  end;
end;
