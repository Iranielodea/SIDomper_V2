...
//  iTotalClientes: Integer;
  ...
  listaEmails: TObjectList<TEmailBase>;
...
//  objEmail := TEmail.Create;
  ...
  listaEmails := TObjectList<TEmailBase>.Create();
  try
...
//        sTexto := sTexto + mmoTexto.Lines[i] + '<br>'; // sem anexo tem que ser com html
    ...
    begin
      try
        ...
        oEmail.IdUsuario := dm.IdUsuario;
        oEmail.Destinatario := Email.Email;
        oEmail.Assunto := edtAssunto.Text;
        oEmail.Texto := sTexto;
        oEmail.ArquivoAnexo := AnexarArquivos;

        listaEmails.Add(oEmail);
...
begin
  ...
  try
    ...
    lblRegistros.Caption := 'Registros: ' + FController.Model.CDSConsulta.RecordCount.ToString();
  finally
    FreeAndNil(Filtro);
  end;
end;
