...
type
  TServerMethods1 = class(TDSServerModule)
    ...
    QDepartamentoCadDep_ModeloImprOrcamento: TIntegerField;
    QProdutoCadProd_Observacao: TStringField;
    QModuloCadMod_Observacao: TStringField;
...
begin
  ...
  try
    ...
    if Result = '' then
      raise Exception.Create('Caminho dos Arquivos para Importação não Informado!');
  finally
    FreeAndNil(obj);
  end;
end;
...
begin
  ...
  try
    ...
    if qry.Fields[0].AsInteger > 0 then
      raise Exception.Create('Duplicação de Registro!');
  finally
    FreeAndNil(qry);
  end;
end;
...
begin
{
  se o usuario tiver revenda no cadastro de usuarios, então retorna zero
  se o usuario nao tiver revenda no cadastro de usuario, então pega a revenda padrao nos parametros
}
  parametros := TParametros.Create;
  Usuario := TUsuario.Create;
  try
...
begin
  ...
  raise Exception.Create('Não Salvo');
...
function TServerMethods1.UsuarioDepartamentoPorUsuario(AIdUsuario: Integer): TJSONValue;
var
  Usuario: TUsuario;
...
begin
  ...
  try
    ...
    Result := TConverte.ObjectToJSON<TUsuarioDepartamentoViewModel>(UsuarioDep);
  finally
    Usuario.DisposeOf;
...
begin
  ...
  try
    if Usuario.HorarioAcessoSistema(AUserName, APassword, AIdUsuario) = False then
      raise Exception.Create('Horário não disponível para usar o SIDomper!');
