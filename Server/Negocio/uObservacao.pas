...
type
  ...
  public
    ...
    procedure LocalizarPadrao(var Qry: TFDQuery; APrograma: Integer); overload;
    procedure LocalizarPadrao(AObservacao: TObservacaoVO; APrograma: Integer); overload;

    procedure LocalizarEmailPadrao(var Qry: TFDQuery; APrograma: Integer); overload;
    procedure LocalizarEmailPadrao(AObservacao: TObservacaoVO; APrograma: Integer); overload;
...
procedure TObservacao.LocalizarEmailPadrao(AObservacao: TObservacaoVO;
  APrograma: Integer);
var
  obj: TFireDAC;
begin
  obj := TFireDAC.create;
  try
    obj.Lista.Add('SELECT Obs_Id, Obs_ObsEmail FROM Observacao');
    obj.Lista.Add(' WHERE Obs_Programa = ' + APrograma.ToString());
    obj.Lista.Add(' AND Obs_EmailPadrao = 1');
    obj.Lista.Add(' AND Obs_Ativo = 1');
    obj.OpenSQL();

    AObservacao.ObsEmail := obj.Model.FieldByName('Obs_ObsEmail').AsString;
...
procedure TObservacao.LocalizarPadrao(AObservacao: TObservacaoVO; APrograma: Integer);
var
  obj: TFireDAC;
begin
  obj := TFireDAC.create;
  try
    obj.Lista.Add('SELECT Obs_Id, Obs_Descricao FROM Observacao');
    obj.Lista.Add(' WHERE Obs_Programa = ' + APrograma.ToString());
    ...
    AObservacao.Id := obj.Model.FieldByName('Obs_Id').AsInteger;
    AObservacao.Descricao := obj.Model.FieldByName('Obs_Descricao').AsString;
  finally
    FreeAndNil(obj);
  end;
end;
