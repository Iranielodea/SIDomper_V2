...
type
  ...
  private
    ...
    procedure MostrarOrcamanto(ALerTodos: Boolean = False);
...
begin
  if FController.Model.CDSItensDepAc_Programa.AsInteger = 1 then
    ...
    MostrarAgendamento()
  else if FController.Model.CDSItensDepAc_Programa.AsInteger = 114 then
    MostrarOrcamanto()
  else begin
...
procedure TfrmDepartamento.DuplicarRegistro1Click(Sender: TObject);
var
  iId: Integer;
begin
  inherited;
  if TFuncoes.Confirmar('Confirmar duplicação do registro?') then
  begin
    iId := FController.Duplicar(dsPesquisa.DataSet.FieldByName('Dep_Id').AsInteger);
    FController.FiltrarId(iId);
//    FController.Editar(iId, Self);
    btnEditar.Click();
  end;
end;
...
procedure TfrmDepartamento.MostrarOrcamanto(ALerTodos: Boolean);
begin
  tsImpressao.TabVisible := True; // FController.MostrarSolicitacao(ALerTodos);
  tsChamado.TabVisible := False;
  tsAtividade.TabVisible := False;
  tsAgendamento.TabVisible := False;
  tsSolicitacao.TabVisible := False;
