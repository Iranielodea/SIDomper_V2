...
begin
  if UsuarioExiste() then
    raise Exception.Create('Já existe usuário cadastrado!');

  if TFuncoes.Confirmar('Confirmar importação de dados?') then
  begin
...
begin
  if not UsuarioExiste() then
    raise Exception.Create('Não existe usuários cadastrados para exportar!');

  if TFuncoes.Confirmar('Confirmar exportação de dados?') then
  begin
    obj := TUtilitarioController.Create;
    try
