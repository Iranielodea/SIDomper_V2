...
{parametro ATipo
  ...
  begin
    TFuncoes.CriarFormularioModal(TfrmObservacao.create(true, FController.TipoOrcamento));
...
begin
  ...
  FController.BuscarTipoOrcamento(FController.Model.CDSCadastroOrc_Usuario.AsInteger);
  inherited;
  Editar(IdOrcamento);
  CarregarContatos;
...
begin
  ...
  try
    ...
    FController.Model.CDSItensModuloOrcIteMod_Descricao.AsString := obj.Model.CDSCadastroMod_Observacao.AsString;
...
begin
  ...
  try
    ...
    FController.Model.CDSItensOrcIte_Descricao.AsString := objProduto.Model.CDSCadastroProd_Observacao.AsString;
...
begin
  ...
  if FController.Model.CDSCadastroOrc_Id.AsInteger > 0 then
  begin
...
begin
  ...
  if AOperacao = operNovo then
    ...
    if FController.Model.CDSItens.RecordCount = 0 then
      raise Exception.Create('Não há Itens para Editar!');
...
// filtro
  ...
  FController.BuscarTipoOrcamento(DM.IdUsuario);
...
begin
  ...
  obj := TfrmOrcamentoImpressao.create(FController.Model.CDSConsultaOrc_Id.AsInteger, AImpressao, AImprimirDireto, FController.TipoOrcamento);
  try
    obj.ShowModal;
  finally
    FreeAndNil(obj);
...
begin
  inherited;
  if TFuncoes.Confirmar('Confirmar Replicação do Orçamento?') then
  begin
...
begin
  ...
  begin
    ...
    if Trim(FController.Model.CDSCadastroOrc_Logradouro.AsString) = '' then
      sErro := sErro + ' Informe o Endereço.' + sLineBreak;
...
begin
  ...
  begin
    ...
    begin
      Result := False;
      raise Exception.Create('Situação Atual já está Faturado');
    end;
