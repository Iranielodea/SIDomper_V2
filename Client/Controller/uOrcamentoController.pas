...
uses
  ...
  uTipoController, uTipoVO, uContatoVO, System.StrUtils;
...
type
  ...
  public
    ...
    procedure OrcamentoNovo(AIdUsuario: Integer);
    function NovoOrcamento(AidUsuario: Integer): TOrcamentoViewModel;
    ...
    procedure ImpressaoModelo2Agro(AIdOrcamento, ACodigoModelo: Integer; APDF: Boolean);
    ...
    procedure BuscarTipoOrcamento(AIdUsuarioOrcamento: Integer);
...
begin
  ...
  begin
    ...
    sTexto := '<html>Orçamento enviado para:<br><br>';
    sTexto := sTexto + 'Número: ' + FormatFloat('000000', Self.Model.CDSCadastroOrc_Numero.AsFloat) + '<br>';
    sTexto := sTexto + 'Razão Social: ' + Self.Model.CDSCadastroOrc_RazaoSocial.AsString + '<br>';
    ...
    sTexto := sTexto + 'Usuário: ' + Self.Model.CDSCadastroUsu_Nome.AsString + '<br>';
    if iSituacao = 4 then // faturado
      sTexto := sTexto + 'Situação: A Faturar'
    else
      sTexto := sTexto + 'Situação: ' + Self.Model.CDSCadastroCSituacao.AsString;
    sTexto := sTexto + '<br><br>';
    ...
    begin
      ...
      begin
        if self.Model.CDSOcorrencia.RecNo = 1 then
          sTexto := sTexto + '<bold>Ocorrências</bold> <br>';

        sTexto := sTexto + 'Data: ' + Self.Model.CDSOcorrenciaOrcOco_Data.AsString + '<br>';
        sTexto := sTexto + 'Usuário: ' + Self.Model.CDSOcorrenciaUsu_Nome.AsString + '<br>';
        sTexto := sTexto + 'Descrição: <br>' + Self.Model.CDSOcorrenciaOrcOco_Descricao.AsString + '<br><br>';
...
begin
  ...
  try
    ...
    begin
      ...
      sTexto := '<html>Orçamento enviado para:<br><br>';
      sTexto := sTexto + 'Número: ' +  FormatFloat('000000', OrcamentoModel.Numero) + '<br>';
      sTexto := sTexto + 'Razão Social: ' +  OrcamentoModel.RazaoSocial + '<br>';
      sTexto := sTexto + 'Contato: ' + OrcamentoModel.Contato + '<br>';
      sTexto := sTexto + 'Usuário: ' + OrcamentoModel.Usuario.Nome + '<br>';
      sTexto := sTexto + 'Situação: ' + Self.RetornarSituacao(OrcamentoModel.Situacao);
      sTexto := sTexto + '<br><br>';
  ...
  //      sEmailUsuario := sEmailUsuario + OrcamentoModel.Usuario.Email + ';';
      ...
      sTexto := sTexto + 'Este orçamento ainda não tem definição. Favor entrar em contato com o Cliente.';
      sTexto := sTexto + '</html>';
...
procedure TOrcamentoController.BuscarTipoOrcamento(
  AIdUsuarioOrcamento: Integer);
var
  usuarioController: TUsuarioController;
  departamentoViewModel: TUsuarioDepartamentoViewModel;
begin
  usuarioController := TUsuarioController.Create;
  try
    departamentoViewModel := usuarioController.ObterUsuarioDepartamento(AIdUsuarioOrcamento);
    if departamentoViewModel.ModeloImpressaoOrcamento = 1 then
      FTipoOrcamento := prOrcamento
    else
      FTipoOrcamento := prOrcamentoAgroD;
  finally
    usuarioController.DisposeOf;
    departamentoViewModel.DisposeOf;
  end;
end;
...
begin
  ...
  begin
    ...
    sAssunto := 'Orçamento: ' + FormatFloat('000000', cNumero) + sCliente ;
  end
  else
    sAssunto := AAssunto;
...
begin
  if FModel.CDSOcorrencia.RecordCount = 0 then
    raise Exception.Create('Não há Itens para Excluir!');
...
//      end
//      else
//        FModel.RelImpressaoSimplesAgroD.print;
...
procedure TOrcamentoController.ImpressaoModelo2Agro(AIdOrcamento,
  ACodigoModelo: Integer; APDF: Boolean);
var
  Negocio: TServerModule2Client;
  sArquivo: string;
...
// 2 - Simplificado
  ...
  FModel.TipoOrcamento := prOrcamentoAgroD;
  ...
  try
    ...
    FModel.CDSImpressao.Open;
    if sTipoRelatorio = 'SIMP' then
      FModel.ImpressaoRelatorioSimplificadoAgroD(sEmail);
    ...
    begin
      sArquivo := ArquivoPDF(ExtractFilePath(Application.ExeName),
                             FModel.CDSImpressaoOrc_Numero.Asstring,
                             FModel.CDSImpressaoOrc_RazaoSocial.AsString);
      FModel.GerarPDFAgroD(sArquivo, sTipoRelatorio);
    end
    else
      FModel.ImpressaoTelaAgroD(sTipoRelatorio);

    if sTipoRelatorio = 'DETA' then
    begin
      FModel.plblOrcamento03.Caption := sEmail;
      FModel.RelImpressaoAgroD.Print;
    end
    else
      FModel.RelImpressaoSimplesAgroD.print;
...
function TOrcamentoController.NovoOrcamento(
  AidUsuario: Integer): TOrcamentoViewModel;
var
  Negocio: TServerModule2Client;
begin
  dm.Conectar;
  Negocio := TServerModule2Client.Create(DM.Conexao.DBXConnection);
  try
    Result := TConverte.JSONToObject<TOrcamentoViewModel>(Negocio.OrcamentoNovo(AIdUsuario));
  finally
    FreeAndNil(Negocio);
  end;
end;
...
var
  ...
  usuarioController: TUsuarioController;
  usuarioViewModel: TUsuarioDepartamentoViewModel;
  tipo: TEnumPrograma;
begin
  usuarioController := TUsuarioController.Create;
  Obs := TObservacaoController.Create;
  try
    usuarioViewModel := UsuarioController.ObterUsuarioDepartamento(dm.IdUsuario);
    if usuarioViewModel.ModeloImpressaoOrcamento = 1 then
      Tipo := prOrcamento
    else
      tipo := prOrcamentoAgroD;

    FTipoOrcamento := tipo;

    Obs.LocalizarPadrao(tipo);
    FModel.CDSCadastroOrc_Observacao.AsString := Obs.Model.CDSCadastroObs_Descricao.AsString;

    Obs.LocalizarEmailPadrao(tipo);
    FModel.CDSCadastroOrc_ObservacaoEmail.AsString := Obs.Model.CDSCadastroObs_ObsEmail.AsString;
  finally
    FreeAndNil(Obs);
    usuarioController.DisposeOf;
    usuarioViewModel.disposeOf;
  end;
end;

procedure TOrcamentoController.OrcamentoNovo(AIdUsuario: Integer);
var
  Negocio: TServerModule2Client;
  orcamentoViewModel: TOrcamentoViewModel;
begin
  dm.Conectar;
  Negocio := TServerModule2Client.Create(DM.Conexao.DBXConnection);
  try
    ...
    orcamentoViewModel := NovoOrcamento(AIdUsuario);

    FModel.CDSCadastro.Append;
    FModel.CDSCadastroOrc_Numero.AsInteger := orcamentoViewModel.Codigo; // CodigoAtual();
    FModel.CDSCadastroOrc_Data.AsDateTime := Date;
    FModel.CDSCadastroOrc_Observacao.AsString := orcamentoViewModel.ObservacaoPadrao;
    FModel.CDSCadastroOrc_ObservacaoEmail.AsString := orcamentoViewModel.TextoEmailPadrao;

    if orcamentoViewModel.ModeloImpressao = 1 then
      FTipoOrcamento := prOrcamento
    else
      FTipoOrcamento := prOrcamentoAgroD;

    if orcamentoViewModel.IdTipo > 0 then
    begin
      FModel.CDSCadastroOrc_Tipo.AsInteger := orcamentoViewModel.IdTipo;
      FModel.CDSCadastroTip_Codigo.AsInteger := orcamentoViewModel.CodTipo;
      FModel.CDSCadastroTip_Nome.AsString := orcamentoViewModel.NomeTipo;
    end;

    FModel.CDSCadastroOrc_Usuario.AsInteger := AIdUsuario;
    FModel.CDSCadastroUsu_Codigo.AsInteger := orcamentoViewModel.CodUsuario;
    FModel.CDSCadastroUsu_Nome.AsString := orcamentoViewModel.NomeUsuario;
...
begin
  {ATipo: 1 - Por Tipo
          2 - Gráfico por Tipo
          3 - Gráfico por Situacao
          4 - Relatorio por Usuario
          5 - Relatorio por não Aprovados
