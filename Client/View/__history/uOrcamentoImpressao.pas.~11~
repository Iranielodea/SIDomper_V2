...
uses
  ...
  uOrcamentoController, Vcl.ComCtrls, uEnumerador;
...
type
  ...
  public
    { Public declarations }
    constructor create(AIdOrcamento: Integer; AImprimir: Boolean; AImprimirDireto: Boolean=False; ATipoOrcamento: TEnumPrograma=prOrcamento);
  end;
...
begin
  FController.TipoOrcamento := FTipoOrcamento;
  if FTipoOrcamento = prOrcamento then
    FController.ImpressaoModelo2(FIdOrcamento, rgImpressao.ItemIndex + 1, False)
  else
    FController.ImpressaoModelo2Agro(FIdOrcamento, rgImpressao.ItemIndex + 1, False);
end;
...
constructor TfrmOrcamentoImpressao.create(AIdOrcamento: Integer; AImprimir: Boolean; AImprimirDireto: Boolean=False; ATipoOrcamento: TEnumPrograma=prOrcamento);
begin
  inherited create(nil);
  FIdOrcamento := AIdOrcamento;
  FController := TOrcamentoController.Create;
  FTipoOrcamento := ATipoOrcamento;
...
begin
  try
    if FTipoOrcamento = prOrcamento then
      ...
      FController.ImpressaoModelo2Agro(FIdOrcamento, rgImpressao.ItemIndex + 1, True);
...
//  Result := sTexto;
  ...
  sTexto := '<html>';
  sTexto := sTexto + 'Olá &nbsp' + FController.Model.CDSCadastroOrc_ContatoCompraVenda.AsString + '<br><br>';
  sTexto := sTexto + FController.Model.CDSCadastroOrc_ObservacaoEmail.AsString + '<br><br>';
  sTexto := sTexto + FController.Model.CDSCadastroUsu_Nome.AsString + '<br><br>';
  sTexto := sTexto + '</html>';
  Result := sTexto;
